...
SELECT
    DATE_FORMAT(STR_TO_DATE(Date, '%d.%m.%Y'), '%Y-%d-%m') AS formatted_date
FROM
   new_schema1.sleep_data;
...
SELECT
    DATE_FORMAT(STR_TO_DATE(WorkoutDay, '%Y-%m-%d'),'%d-%m-%Y') AS formatted_date
FROM
   new_schema1.training_data;
...
SELECT
WorkoutDay, Date  AS date
FROM
    new_schema1.training_data t1, new_schema1.sleep_data
LEFT JOIN
...
#duplicates
-- Step 1: Create a temporary table to store the IDs of the duplicate rows
...
-- Step 2: Update the duplicate cells to NULL
UPDATE my_table
SET date = NULL
